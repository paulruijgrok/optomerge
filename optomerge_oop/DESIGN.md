# An object-oriented design for optomerge

This is a proposal, not a rewrite. Nothing in `optomerge/` is touched. The
skeleton lives in `optomerge_oop/optomerge_oo/` as classes with documented
stubs, so the shape of the design can be reviewed before any logic moves.
